...
<head>\
    ...
    <script defer src='/build/bundle1.js'></script>\
</head>\
<body>\
</body>\
</html>\
...
{
    StreamString temp;
    temp.reserve(200); // Preallocate a large chunk to avoid memory fragmentation
    temp = "console.log('hello world');";
    server.send(200, "text/javascript", temp.c_str());
}

void handleJS()
{
    // StreamString temp;
    //temp.reserve(27000); // Preallocate a large chunk to avoid memory fragmentation
    //temp = "";
...
{
    StreamString temp;
    temp.reserve(300); // Preallocate a large chunk to avoid memory fragmentation
    temp = ".settings-form.svelte-1aqasb2 label.svelte-1aqasb2{display:flex;justify-content:space-between}.settings-form.svelte-1aqasb2 input.svelte-1aqasb2{width:50%}\
.main.svelte-xx7voz{width:800px;margin:0 auto}nav.svelte-xx7voz{display:flex;gap:1rem;margin-bottom:1rem}\
";
    server.send(200, "text/css", temp.c_str());
}

void handleGlobalCSS()
{
    StreamString temp;
    temp.reserve(1000); // Preallocate a large chunk to avoid memory fragmentation
    temp = "\
html, body {\
	position: relative;\
	width: 100%;\
...
void WebServer::setup() {
    ...
    server.on("/build/bundle1.js", handleJS1);
